...
CREATE_USER_URL = reverse('user:create')  # user as app, create as endpoint
...
class PublicUserApiTests(TestCase):
    ...
    def test_password_too_short_error(self):
        ...
        user_exists = get_user_model().objects.filter(
            email=payload['email']
        ).exists()  # make sure user isn't created in db
...
class PrivateUserApiTests(TestCase):  # break out test bc auth is done before all tests
    ...
    def test_post_me_not_allowed(self):
        ...
        self.assertEqual(res.status_code, status.HTTP_405_METHOD_NOT_ALLOWED)  # post should only be used with create_user api
