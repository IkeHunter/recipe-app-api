...
def recipe_image_file_path(instance, filename):
    ...
    ext = os.path.splitext(filename)[1]  # strip off extension
    filename = f'{uuid.uuid4()}{ext}'

    return os.path.join('uploads', 'recipe', filename)  # ensures path is created correctly for filesystem
...
class Recipe(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,  # defined in settings.py, changes automatically when user model changes
